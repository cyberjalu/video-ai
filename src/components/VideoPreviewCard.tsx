...
import { Copy, Download, FolderOpen, RotateCcw, Video, Volume2, VolumeX, Play, AlertTriangle } from "lucide-react";
      ...
      <Card className="overflow-hidden p-5">
        ...
        <div className="mt-5 flex justify-center">
          <div className="relative">
            <div className="absolute -inset-6 rounded-[48px] bg-gradient-to-b from-sky-500/12 via-cyan-500/10 to-transparent blur-2xl" />
                ...
                >
                  <video
                    ...
                    onError={() => {
                      setReady(false);
                      setLoadError(
                        "Could not load local video in the preview. Open the output folder to play the MP4.",
                      );
                    }}
                  />

                  {loadError ? (
                    <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-2 bg-black/80 px-4 text-center">
                      <AlertTriangle className="h-5 w-5 text-amber-300" />
                      <div className="text-[11px] leading-relaxed text-zinc-300">{loadError}</div>
                      {outputDir ? (
                        <SecondaryButton
                          type="button"
                          className="mt-1 h-7 px-2.5 text-[10px]"
                          onClick={(e) => {
                            e.stopPropagation();
                            void revealItemInDir(outputDir);
                          }}
                        >
                          <FolderOpen className="h-3.5 w-3.5" />
                          Open folder
                  ...
                  {!ready && !loadError ? (
                    <div className="absolute inset-0 z-10 grid place-items-center bg-zinc-950/80">
        ...
        {(title || source) && (
          <div className="mt-5">
            ...
            {source && <div className="mt-1 text-xs text-zinc-500">{source}</div>}
          </div>
        )}
